...
var pollTimer = {};
var pollNoms = [];
var RPOpts = ['freeroam', 'citylife', 'wildlife', 'goodvsevil', 'conquest', 'trainer', 'pokehigh', 'totaldramaisland', 'prom', 'cruise', 'murdermystery', 'pokemonmysterydungeon', 'dungeonsndragonites', 'kingdom', 'hungergames', 'zombieapocalypse'];
var rpcaps = ['Freeroam', 'City Life', 'Wild Life', 'Conquest', 'Trainer', 'PokeHigh', 'Total Drama Island', 'Prom', 'Cruise', 'Murder Mystery', 'Pokemon Mystery Dungeon', 'Dungeons \'n Dragonites', 'Kingdom', 'Hunger Games', 'Zombie Apocalypse'];
...
//		var text = config.excepts.indexOf(toId(by)) < 0 ? '/pm ' + by + ', ' : '';
	...
	start: function(arg, by, room) {
		...
		if (config.serverid == 'sora' && !(room == "amphyrp")){
	...
	endrp: function(arg, by, room) {
		...
		if (config.serverid === 'sora' && this.RP[room].setAt) {
	...
	setvoid: function(arg, by, room) {
		if (config.serverid !== 'sora' || !(room in this.RP) || this.RP[room].plot || !arg || !this.hasRank(by, '%@#&~')) return false;
		var spl = arg.split(', ');
		console.log(spl);
	...
	void: function(arg, by, room) {
		if (config.serverid !== 'sora' || !(room in this.RP) || this.RP[room].plot) return false;
		var text = '';
		if (room === 'rustyrp'){
			text += '**';
	...
	voice: function(arg, by, room) {
		if (config.serverid !== 'sora' || !('amphyrp' in this.RP) || room.charAt(0) !== ',') return false;
		var text = '/pm ' + by + ', ';
		return this.say(room, text + 'The command ".voice" has been deprecated, please PM a mod for voice.');
	},
	site: function(arg, by, room) {
		if (config.serverid !== 'sora') return false;
		if ((this.hasRank(by, '+%@#~') && config.rprooms.indexOf(room) !== -1) || room.charAt(0) === ',') {
			var text = '';
		} else {
			var text = '/pm ' + by + ', ';
		}
		this.say(room, text + 'Sora Roleplaying\'s Website: http://soraroleplaying.weebly.com/');
	},
    rppoll: function(arg, by, room) {
		...
		this.say(room, '/wall **PM Snowflake Bot** with .nom [RP] to nominate the RP you want to be next. **Remember to only PM RPs you can host** and PM staff your customs. Ends at xx:' + ((((now.getMinutes()+3)%60) < 10) ? '0' + (((now.getMinutes()+3)%60).toString()) : ((now.getMinutes()+3)%60).toString()) + ':' + (((now.getSeconds() < 10)) ? '0' + now.getSeconds().toString() : now.getSeconds().toString()));
    ...
    nominate: function(arg, by, room) {
    	...
    	if (pollRoom == 'roleplaying' && toId(arg) == 'freeroam'){
...
//		if (!arg) return this.say(room, 'Please specify the requester of the poll.');
	...
	legends: function(arg, by, room) {
		if (config.serverid == 'sora' && room.charAt(0) === ',') {
			if (arg) {
		...
		if (config.serverid !== 'sora' || !this.hasRank(by, '%@#&~') || !(room in this.RP))
		{
			var text = '/w '+ by + ',';
		} else {
			var text = '';
	...
	nominators: function (arg, by, room) {
		if (config.serverid !== 'sora' || !this.hasRank(by, '~') || !(room in this.RP)) return false;
	...
	publicserviceannouncement: function(arg, by, room) {
		if (config.serverid == '	' && room.charAt(0) === ',') {
